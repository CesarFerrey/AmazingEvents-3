...
function cardsDom(data) {
  ...
  for (let i = 0; i < data.events.length; i++) {
      main += `
        <div class="event_list card col-12 col-sm-3 col-md-6 col  m-2" style="width: 16rem;">
        <img src="${data.events[i].image}" class="card-img-top" alt="...">
        <div class="card-body">

        <h5 class="card-title">${data.events[i].name}</h5>
        <h5 class="category d-none">${data.events[i].category}</h5>

        <p class="card-text">${data.events[i].description}</p>
         
          <div>
            <h6>Lorem.</h6>      
            <a href="./pages/details.html?name=${data.events[i].name}" class="btn btn-primary" >Go somewhere</a>
          </div>        
          </div>
      </div>
...
const data_base = data.events.map((eventos) => eventos);
